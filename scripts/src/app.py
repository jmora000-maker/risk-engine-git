...
with col1:
    ...
    st.text(f"Targeting project folder")
    ...
    # 1. Place the button here first
    start_pipeline = st.button("Run Risk Engine Pipeline", use_container_width=True, type="primary")

    # 2. Place the Subheader and Live Console Logs below it
    st.subheader("Live Operational Console Logs")
    log_placeholder = st.empty()

    # Pre-populate with previous logs if they exist
    log_placeholder.code(st.session_state.logs)

# FIX 2: Define Column 2 BEFORE the pipeline execution logic
with col2:
...
# --- PIPELINE EXECUTION ---
# This can now run safely. Because columns are established above,
# log_placeholder.code() updates won't cause Column 2 to vanish.
if start_pipeline:
    buffer = io.StringIO()
